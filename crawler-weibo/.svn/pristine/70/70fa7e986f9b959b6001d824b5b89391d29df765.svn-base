...
public class SinaOauthRefresh {
	...
	private static void refresh(HttpClient client, Oauth sinaOauth)
			throws Exception {
		String code = "";
		String url = "https://api.weibo.com/oauth2/authorize";/*模拟登录的地址，https://api.weibo.com/oauth2/authorize*/
		...
		if (location != null) {
			...
			Matcher m = Pattern.compile("state=([^&]*?)&code=([^&]*+)").matcher(retUrl);
			if (m.find()) {
				state = m.group(1);
				code = m.group(2);
			}
	...
	public static void refresh(Oauth... sinaOauthList)
			throws Exception {
