...
public class TwitterOauth {
	...
	public  void generateSignature(WeiboPage weiboPage) {
	    ...
	    oAuth.setOAuthConsumer(oauth.getConsumerKey(), oauth.getConsumerSecret());
	    oAuth.setOAuthAccessToken(new AccessToken(oauth.getAccessToken(), oauth.getAccessTokenSecret()));
        String signature = "";
        String method = weiboPage.getHttpMethod() == null ? "GET" : weiboPage.getHttpMethod().toString();
        String queryString = (weiboPage.getQuery()==null)? "":weiboPage.getQuery().toString();
        String url = (queryString == null || "".equals(queryString)) ? weiboPage.getUrl() : weiboPage.getUrl() + "?" + queryString;
        try {
			signature = oAuth.generateAuthorizationHeader(method, url, null, oAuth.getOAuthAccessToken());
	...
	public  String generateSignature(String url, String queryStr) {
		...
		try {
			signature = oAuth.generateAuthorizationHeader("POST", url + "?" + queryStr , null, oAuth.getOAuthAccessToken());
		} catch (TwitterException e) {
			e.printStackTrace();
		}
		return signature;
