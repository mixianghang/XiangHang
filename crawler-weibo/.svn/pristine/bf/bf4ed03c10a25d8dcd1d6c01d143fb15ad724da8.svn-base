...
public class SinaWeiboExtractor extends JsonExtractor {
  ...
  public WeiboPage process(WeiboPage weiboPage) {
    ...
    if (jsonObject.containsKey("error_code")) return weiboPage;

    JSONArray infoJSONArray = jsonObject.getJSONArray("statuses");

    if (infoJSONArray.isEmpty()) return weiboPage;
    ...
    while (iterator.hasNext()) {
      ...
      if (json.containsKey("retweeted_status")) {
        JSONObject retweeted_statusJson = json.getJSONObject("retweeted_status");
    ...
    log.info("SinaWeiboExtractor 处理完成");
