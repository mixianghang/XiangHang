...
public class NeteaseSearchExtractor extends JsonExtractor {
	...
	public WeiboPage  process(WeiboPage  weiboPage) {
		...
		try {
			log.info("NeteastSearchExtractor...... ");
			List<VelocityBean> beans = new ArrayList<VelocityBean>();
      JSONArray result = JSONArray.fromObject(weiboPage.getContent());
			for(int i=0; i< result.size(); i++) {
				try {
					VelocityBean bean = VelocityBeanPool.borrowObject();
					JSONObject weibo = result.getJSONObject(i);
