...
public enum Task {
	Sina(2,VariableCache.Task.sinaRateControl, VariableCache.Task.sinaFriendShipProcessInterval),
	Tencent(3,VariableCache.Task.tencentRateControl),
	Twitter(1,VariableCache.Task.twitterRateControl, VariableCache.Task.twitterFriendShipProcessInterval),
	Sohu(4,VariableCache.Task.sohuRateControl),
	Netease(5,VariableCache.Task.neteaseRateControl),
	All(0,0L);
	int i = 0;
	/**
	 * 访问频率控制,单位ms
	...
	public static long getMinRateControl(){
		long minRateControl = Math.min(Task.Sina.getRateControl(), Task.Tencent.getRateControl());
		minRateControl = Math.min(minRateControl, Task.Twitter.getRateControl());
		minRateControl = Math.min(minRateControl, Task.Sohu.getRateControl());
		minRateControl = Math.min(minRateControl, Task.Netease.getRateControl());
