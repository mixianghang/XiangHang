		...
		for (String fileName : attachmentsFileNames) {
			if (fileName != null && !"".equals(fileName)) {
				...
				try {
					...
					/**
					 * 加上文件名将作为附件发送,否则将作为信件的文本内容;MimeUtility.encodeText()
					 * 函数是把中文的文件名转换为base64编码, 可以用MimeUtility.decodeText()解码;
					 * 只有编码后你所发送的邮件才能被其他软件正确解析否则乱码!
					 */

					int position = (fileName.lastIndexOf("\\") != -1) ? fileName
							.lastIndexOf("\\") + 1
							: fileName.lastIndexOf("/") + 1;
					String attachmentName = fileName.substring(position,
							fileName.length());
