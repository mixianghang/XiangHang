...
public class SohuUserIdRule extends AbstractRule {
	...
	public boolean rule(WeiboPage weiboPage) {
		if (weiboPage.getVelocityBeanList().size() > 0) {
			...
			WeiboPage friendWeiboPage = GenWeiboPageFromAccount
					.ofSohuFriend(account);
			WeiboPage followerWeiboPage = GenWeiboPageFromAccount
					.ofSohuFollower(account);
			weiboPage.setSonWeiboPages(new WeiboPage[] { friendWeiboPage,
					followerWeiboPage });
