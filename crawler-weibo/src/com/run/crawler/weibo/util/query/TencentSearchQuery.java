...
public class TencentSearchQuery extends Query {
	...
	/**
	 * 消息的正文类型（按位使用） 0-所有，0x01-纯文本，0x02-包含url，0x04-包含图片，0x08-包含视频，0x10-包含音频
	 */
	private String contenttype = "0";

	/**
	 * 排序方式 0-表示按默认方式排序(即时间排序(最新))
	 */
	private String sorttype = "0";

	/**
	 * 消息的类型（按位使用） 0-所有，1-原创发表，2 转载，8-回复(针对一个消息，进行对话)，0x10-空回(点击客人页，进行对话)
	 */
	private String msgtype = "0";

	/**
	 * 搜索类型 0-默认搜索类型（现在为模糊搜索）
	 * 1-模糊搜索：时间参数starttime和endtime间隔小于一小时，时间参数会调整为starttime前endtime后的整点
	 * ，即调整间隔为1小时 8-实时搜索：选择实时搜索，只返回最近几分钟的微博，时间参数需要设置为最近的几分钟范围内才生效，并且不会调整参数间隔
	 */
